...
export default function Diagnosis() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Step 1: Upload file to Cloudinary if one is selected
      let fileUrl = '';
      if (selectedFile) {
        try {
          const uploadResponse = await uploadFile(selectedFile, 'report');
          fileUrl = uploadResponse.url; // This is the Cloudinary secure_url
        } catch (uploadErr) {
      ...
      // Step 2: Create the visit/medical record with Cloudinary URL
      const recordData = {
        patient: patientId,
        title: diag,
        ...
        <div className="md:col-span-2 space-y-6 flex flex-col h-full">
            ...
            <div className="space-y-5">
              ...
              <div className="form-group">
                ...
                <textarea 
                  ...
                  value={rx} 
                  onChange={e => { setRx(e.target.value); setSafetyCheck(null); }}
          ...
          <div className="flex gap-3">
            <button 
              type="submit" 
              disabled={isSaving} 
              className="flex-1 py-4 bg-teal-600 text-white font-bold rounded-xl shadow-md hover:bg-teal-700 hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed text-base">
              {isSaving 
